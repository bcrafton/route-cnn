...
def route_block(x, nexpert, route, expert):
    blocks = [x]
    for (c, f, p) in route:
    ...
    route  = tf.squeeze(dense(flat, [f, nexpert]))
...
train_out, test_out, pi, train_idx, test_idx = route_block(x=x, nexpert=20, route=[[3, 64, 2], [64, 64, 2], [64, 64, 2], [64, 64, 4]], expert=[[3, 64, 2], [64, 64, 2], [64, 64, 2], [64, 64, 4]])
...
for ii in range(epochs):
    ...
    for jj in range(0, 10000, batch_size):
        ...
        [i, _sum_correct] = sess.run([test_idx, sum_correct], feed_dict={x: xs, y: ys})
        idxs[i] += 1 
